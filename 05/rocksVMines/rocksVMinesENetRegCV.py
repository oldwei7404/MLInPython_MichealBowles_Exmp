...
for ixval in range(nxval):
    ...
    if ixval == 0:
        ...
        yOut = numpy.concatenate((yTemp, labelTest), axis=0) # record of labels for each fold, in a single vector

        #accumulate predictions
        predTemp = numpy.array(pred)
        pred = numpy.concatenate((predTemp, numpy.dot(xTest, coefs)), axis = 0) # record of pred for each fold, 2D array, row = fold * no. per fold, concatenate in row direction


#calculate miss classification error
misClassRate = []
_,nPred = pred.shape
for iPred in range(1, nPred):  # nPred (column) is no. of attributes
...
plotAlphas = list(alphas[1:len(alphas)]) # excluding 0th element
